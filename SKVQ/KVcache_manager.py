...
class ModelKVCacheManager:
    ...
    def create(
        ...
        for i, _ in enumerate(layers):
            ...
            if turboquant_config is not None:
                if not fake_quant:
                    raise ValueError("TurboQuant backend currently supports fake_quant=True only")
                from turboquant_backend import TurboQuantBackend

                tq_cfg = dict(turboquant_config)
                tq_cfg.setdefault("head_dim", head_dim)
                tq_cfg.setdefault("hidden", hidden)
                tq_cfg.setdefault("key_bits", kbits)
                tq_cfg.setdefault("value_bits", vbits)
                tq_cfg.setdefault("layer_idx", i)
                tq_cfg.setdefault("num_layers", len(layers))
                tq_cfg.setdefault("group_size", gsize if tq_cfg.get("use_reorder", False) else None)
                tq_cfg.setdefault("clipping", clipping)
                processor_cfg["tq_backend"] = TurboQuantBackend(**tq_cfg)
            kv_managers.append(
    ...
    def __str__(self) -> str:
        ...
        return (
            ...
            f">>> TurboQuant:   {manager.KV_processor.tq_backend is not None}\n"
    ...
    def tag(self) -> str:
        ...
        tq = processor.tq_backend.tag if processor.tq_backend is not None else ""

        return f"k{kbits}-v{vbits}-g{gsize}-w{window}{reorder}{smooth}{clipping}{pre_rope}{KIVI_mode}{sink}{fp8}{h2o}{rand}{tq}"
