...
class SKVQuantProcessor(nn.Module):
    ...
    def quantization(
        ...
        if self.KIVI_mode:
            impl = "py"
        if impl == "cuda" and skvq_quant is None:
            if not self.fake_quant:
                raise ImportError("skvq_quant is required for packed CUDA quantization")
            impl = "py"
    ...
    def dequant(
        ...
        if impl == "cuda" and skvq_quant is None:
            raise ImportError("skvq_quant is required for CUDA dequantization")
