...
python -m turboquant.block_cache.eval_ppl \
  ...
  --max-samples 64 --max-tokens 8192 --seq-len 1024 --stride 512 --policy hybrid \
  ...
  --max-cached-decompressed-blocks 128 --output "$BASE/server_ppl/ppl.jsonl"
...
python -m turboquant.block_cache.eval_longbench \
  ...
  --protected-layers 1 --protected-key-bits 8 --protected-value-bits 8 \
  --key-group-size 128 --value-group-size 64 --reorder-file "$REORDER" \
  --max-cached-decompressed-blocks 128 --output-dir "$BASE/server_longbench"
